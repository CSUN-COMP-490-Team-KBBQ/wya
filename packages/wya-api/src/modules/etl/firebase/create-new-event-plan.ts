...
export const etlFirebaseCreateNewEventPlan = async (
    ...
    try {
        ...
        await firebaseFirestore.runTransaction(async (transaction) => {
            ...
            var inviteesByUserIdsAndHost = inviteesByUserIds;
            inviteesByUserIdsAndHost.push(hostId);
            await Promise.all(
                inviteesByUserIdsAndHost.map((userId) => {
                    const eventPlanAvailabilitiesHeatMapRef =
                        firebaseFirestore.doc(
                            `/${process.env.EVENT_PLANS}/${eventPlanId}/${process.env.EVENT_PLAN_AVAILABILITIES}/${userId}`
                        );
                    return transaction.create(
                        eventPlanAvailabilitiesHeatMapRef,
                        {
            ...
            await Promise.all(
                inviteesByUserIds.map((userId) => {
                    const eventPlanInviteeAvailabilityRef =
                        firebaseFirestore.doc(
                            `/${process.env.EVENT_PLANS}/${eventPlanId}/${process.env.EVENT_PLAN_AVAILABILITIES}/${userId}`
                        );
